    ...
    ? `
      ...
      ${searches.slice(0, 10).map((query) => `
        <span class="recent-chip">
          <button class="recent-chip-label" type="button" data-recent-search-action="apply" data-query="${escapeAttr(query)}">${escapeHtml(query)}</button>
          <button class="recent-chip-delete" type="button" aria-label="최근 검색어 삭제" title="삭제" data-recent-search-action="remove" data-query="${escapeAttr(query)}">
    ...
    return `
      ...
      >
        ...
        <div class="page-card-body">
          ${page.originalImage ? `<img class="page-image" src="${escapeAttr(page.originalImage)}" alt="p.${page.pageNumber} 첨부 이미지">` : ''}
...
function appendPageToBook(bookId, pageData) {
  ...
  const page = DB.add('pages', {
    ...
    originalImage: pageData.originalImage || null,
...
function saveScannedPage() {
  ...
  const previewUrl = document.getElementById('scan-preview-img')?.src || '';
  ...
  const matches = syncScanSaveChoices();
  ...
  const pageData = {
    originalImage: previewUrl.startsWith('data:image/') ? previewUrl : null,
    extractedText: text,
    processedText: text,
...
function initRecentSearchEvents() {
  document.addEventListener('click', (event) => {
    const button = event.target.closest('[data-recent-search-action]');
    if (!button) return;

    const query = button.dataset.query || '';
    if (button.dataset.recentSearchAction === 'apply') {
      applySearch(query);
    }
    if (button.dataset.recentSearchAction === 'remove') {
...
function initScanModalEvents() {
  const closeScanModal = () => { closeModal('modal-scan'); resetScanSaveChoices(); resetScanForm(); };
